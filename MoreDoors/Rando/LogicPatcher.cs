...
public static class LogicPatcher
{
    ...
    private static bool ResolveMoreDoorsRando(string term, out int result)
    {
        if (ParseMoreDoorsTerm(term, out var doorName))
        {
            result =
                (
                    RandoInterop.IsEnabled
                    && RandoInterop.LS != null
                    && RandoInterop.LS.EnabledDoorNames.Contains(doorName)
                )
                    ? 1
                    : 0;
            return true;
    ...
    private static StartModifier Forbid(params string[] doorNames) =>
        ForbidUnless("FALSE", doorNames);

    private static StartModifier ForbidUnlessRoomRando(params string[] doorNames) =>
        ForbidUnless("ROOMRANDO", doorNames);
    ...
    private static void PatchStartLocations(Dictionary<string, StartDef> startDefs)
    {
        ...
        foreach (var startName in keys)
        {
            ...
            if (sceneToDoors.TryGetValue(start.SceneName, out var doors))
                startDefs[startName] = Forbid([.. doors])(startDefs[startName]);
            if (StartModifiers.TryGetValue(startName, out var modifier))
                startDefs[startName] = modifier(startDefs[startName]);
        }
    }

    private static void HandleDoorLogic(
    ...
    )
    {
        ...
        replacementMap[$"{doorLoc.TransitionName}/"] = new NameToken(
            $"{doorLoc.TransitionProxyName}/"
        );

        lmb.GetOrAddTerm(doorLoc.TransitionName, TermType.State);
        lmb.AddWaypoint(
            new(doorLoc.TransitionProxyName, lmb.LogicLookup[doorLoc.TransitionName].ToInfix())
        );
        ...
        else
        {
            ...
            lmb.DoLogicEdit(
                new(
                    doorLoc.TransitionProxyName,
                    $"ORIG + {data.KeyTermName} | {doorLoc.TransitionName}{lanternClause} + {data.KeyTermName}"
                )
            );
        }
        ...
        lmb.AddLogicDef(
            new(
                doorLoc.TransitionName,
                $"{doorLoc.TransitionName} | {doorLoc.TransitionProxyName}{lanternClause} + {data.KeyTermName}"
            )
        );
    }
    ...
    public static void ModifyCoreDefinitions(GenerationSettings gs, LogicManagerBuilder lmb)
    {
        ...
        foreach (var e in DoorData.All())
        {
            ...
            if (ls.IncludeDoor(doorName))
            {
                ...
                lmb.AddItem(
                    new StringItemTemplate(
                        data.Key!.ItemName,
                        $"`{data.KeyTermName}<1` => {MORE_KEYS_TERM}++ >> {data.KeyTermName}++"
                    )
                );
                ...
                if (doorName == DoorNames.FALSE)
                {
                    ...
                    lmb.DoLogicEdit(
                        new("Crossroads_10[left1]", "ORIG + (ROOMRANDO | Defeated_False_Knight)")
                    );
