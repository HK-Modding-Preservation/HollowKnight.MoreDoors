...
public class MoreDoors : Mod, IGlobalSettings<GlobalSettings>, ICustomMenuMod
{
    ...
    public static new void LogDebug(string msg)
    {
        ((Loggable)Instance!).LogDebug(msg);
    }

    public static new void Log(string msg)
    {
        ((Loggable)Instance!).Log(msg);
    }

    public static new void LogWarn(string msg)
    {
        ((Loggable)Instance!).LogWarn(msg);
    }

    public static new void LogError(string msg)
    {
        ((Loggable)Instance!).LogError(msg);
    }
    ...
    )
    {
        ...
        if (ModHooks.GetMod("ExtraRando") is Mod)
            ExtraRandoInterop.ExtraRandoInterop.Setup();
        if (ModHooks.GetMod("FStatsMod") is Mod)
            FStatsInterop.FStatsInterop.Setup();
        if (ModHooks.GetMod("Randomizer 4") is Mod)
            RandoInterop.Setup();
    }

    public override List<(string, string)> GetPreloadNames() =>
        [.. IC.Preloader.Instance.GetPreloadNames()];
